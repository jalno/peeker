...
class WordpressScript extends Script
{
        ...
        if (!$this->dbInfo) {
            ...
            foreach (['DB_NAME', 'DB_USER', 'DB_PASSWORD', 'DB_HOST'] as $const) {
                ...
                if (preg_match("/define\(\s*[\'|\"]{$const}[\'|\"],\s*[\'|\"]([^\"|^\']+)[\'|\"]\s*\);/", $content, $matches)) {
                    $log->reply($matches[1]);
                    switch ($const) {
                        case 'DB_NAME':$dbInfo['database'] = $matches[1];
                            break;
                        case 'DB_USER':$dbInfo['username'] = $matches[1];
                            break;
                        case 'DB_PASSWORD':$dbInfo['password'] = $matches[1];
                            break;
                        case 'DB_HOST':$dbInfo['host'] = $matches[1];
                            break;
                    }
                } else {
    ...
    {
        return $this->requireDB()->replace('options', [
            'option_name' => $name,
            'option_value' => $value,
            'autoload' => 'yes',
        ]);
