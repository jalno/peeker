...
class WordpressScanner extends Scanner
{
    ...
    public function checkFile(File $file): ?IAction
    {
        ...
        if (!$original->exists()) {
            ...
            if (
                preg_match("/^wp-content\/.+\.php$/", $path)
                and !preg_match("/^wp-content\/(mu-plugins\/.+|themes\/.+|plugins\/.+|wp-rocket-config\/.+|advanced-cache|index)\.php$/", $path)
            ) {
                return (new actions\RemoveFile($file))
                    ->setReason('extra-php-file-in-wp-content');
            }
            if ('php' == $ext and !preg_match("/^(wp-(admin|includes|content)\/.*)?[^\/]+\.php$/", $path)) {
                return (new actions\RemoveFile($file))
    ...
    protected function download(string $version): Directory
    {
        ...
        (new Client())->get("https://wordpress.org/wordpress-{$version}.zip", [
            'save_as' => $zipFile,
        ]);
        $zip = new \ZipArchive();
        if (false === $zip->open($zipFile->getPath())) {
