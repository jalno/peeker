...
class ActionManager {
	...
	public function doActions(?bool $onlyInteractive = null): void {
		...
		for ($x = 0, $l = count($this->actions); $x < $l; $x++) {
			...
			if ($onlyInteractive === null or (($action instanceof IActionInteractive and $action->hasQuestions()) === $onlyInteractive)) {
	...
	public function hasActions(?bool $onlyInteractive = null): bool {
		foreach ($this->actions as $action) {
			if ($onlyInteractive === null or $action instanceof IActionInteractive == $onlyInteractive) {
				return true;
			}
		}
		return false;
